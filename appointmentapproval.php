...
if(isset($_POST['submit']))
{
    ...
    {
        $sql ="UPDATE patient SET status='Actif' WHERE patientid='$_GET[patientid]'";
        $qsql=mysqli_query($con,$sql);

        $sql ="UPDATE appointment SET appointmenttype='en ligne',departmentid='$_POST[select5]',doctorid='$_POST[select6]',status='Approuvé',appointmentdate='$_POST[appointmentdate]',appointmenttime='$_POST[time]' WHERE appointmentid='$_GET[editid]'";
    ...
    {
        $sql ="UPDATE patient SET status='Actif' WHERE patientid='$_POST[select4]'";
        $qsql=mysqli_query($con,$sql);

        $sql ="INSERT INTO appointment(appointmenttype,patientid,departmentid,appointmentdate,appointmenttime,doctorid,status) 
        VALUES('En ligne','$_POST[select4]','$_POST[select5]','$_POST[appointmentdate]','$_POST[time]','$_POST[select6]','$_POST[select]')";
        
        if($qsql = mysqli_query($con,$sql))
        {
            echo "<script>alert('Rendez-vous enregistré avec succès...');</script>";
        }
        else
        {
            echo mysqli_error($con);
...
<div class="card">
    <div class="block-header">
        <h2 class="text-center">Processus d'approbation des rendez-vous</h2>
    </div>
        ...
        <table class="table table-striped">                
            <tr>
                ...
                <td>
                    ...
                    {
                        $sqlpatient= "SELECT * FROM patient WHERE status='Actif'";
                        $qsqlpatient = mysqli_query($con,$sqlpatient);
            ...
            <tr>
                ...
                <td>
                    <select name="select5" id="select5" class="form-control show-tick">
                        ...
                        <?php
                        $sqldepartment= "SELECT * FROM department WHERE status='Actif'";
            ...
            <tr>
                ...
                <td>
                    <select name="select6" id="select6" class="form-control show-tick">
                        <option value="">Sélectionner</option>
                        <?php
                        $sqldoctor= "SELECT * FROM doctor INNER JOIN department ON department.departmentid=doctor.departmentid WHERE doctor.status='Actif'";
